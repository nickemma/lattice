...
curl -fsS -X POST "$base_url/v1/documents" \
  ...
  -d "{\"id\":\"$document_id\",\"title\":\"Compose smoke\",\"body\":\"Kafka OpenSearch end to end verification\",\"tags\":[\"compose\",\"smoke\"]}" \
  | grep -q '"accepted":true'
poll_for_id 'Kafka OpenSearch' "$document_id"
...
curl -fsS -X POST "$base_url/v1/documents" \
  -H 'content-type: application/json' \
  -d "{\"id\":\"$replay_id\",\"title\":\"Replay smoke\",\"body\":\"Buffered Kafka message after consumer restart\",\"tags\":[\"replay\"]}" \
  | grep -q '"accepted":true'
docker compose -f "$compose_file" up -d lattice-indexer >/dev/null
poll_for_id 'Buffered Kafka' "$replay_id"
